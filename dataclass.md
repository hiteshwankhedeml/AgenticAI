...
* `@dataclass` is a decorator that automatically generates common methods for a class.
* It removes boilerplate code like writing `__init__()` manually.
* You only define the attributes; Python creates the rest.
* Best used for classes that mainly hold data.
